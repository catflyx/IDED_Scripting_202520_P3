...
namespace TestProject1
{
    ...
    {
        //  Calcula daño según si el movimiento es físico o especial
        private double CalcularDanio(
    Pokemon atacante, Pokemon defensor, Move movimiento, double mod)
        {
            // Si el modificador es 0 (inmunidad), el daño es 0
            if (mod == 0)
                return 0;

            // Usa ataque/defensa física o especial según el tipo de movimiento
            ...
            // Fórmula base del daño (según el enunciado del profe)
            ...
            //  Ajuste importante: usar redondeo normal (no truncar hacia abajo)
            // Esto corrige los casos donde había diferencia de 1 punto
        ...
        public void DebeCalcularDanioConInstancias(
            int level, int basePower, int atk, int def, double mod, double esperado)
        {
            //  Crear Pokémon atacante y defensor con los stats del caso
            ...
            //  Crear el movimiento
            var moveType = (level % 2 == 0) ? Move.MoveType.Special : Move.MoveType.Physical;
            var move = new Move("Ataque", PokemonType.Fire, moveType, basePower);

            //  Calcular daño
            double dmg = CalcularDanio(attacker, defender, move, mod);

            Assert.That(dmg, Is.EqualTo(esperado),
                $"Error en caso LV:{level} PWR:{basePower} ATK:{atk} DEF:{def} MOD:{mod}");
        }
    }
}
