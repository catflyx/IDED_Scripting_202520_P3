...
{
    [TestFixture]
    public class DamageCalculatorPhysicalTests
    {
        private double CalcularDanioFisico(Pokemon atacante, Pokemon defensor, Move movimiento, double mod)
        {
            if (mod == 0)
                return 0;

            double atk = atacante.Attack;
            double def = defensor.Defense;
        ...
        public void DebeCalcularDanioFisico(
            int level, int basePower, int atk, int def, double mod, double esperado)
        {
            ...
            var move = new Move("Golpe Físico", PokemonType.Fire, Move.MoveType.Physical, basePower);

            double dmg = CalcularDanioFisico(attacker, defender, move, mod);

            Assert.That(dmg, Is.EqualTo(esperado).Within(1),
                $"Error físico → LV:{level} ATK:{atk} DEF:{def} MOD:{mod}");
        }
    }
}
